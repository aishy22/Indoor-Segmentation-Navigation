...
class ColorCodedCostMapGenerator:
    ...
    # Colors stored in RGB order (safe for matplotlib).
    ...
    def __init__(self, model_path, device='cuda'):
        ...
        self.model = smp.Unet('resnet50', encoder_weights=None,
                              classes=4).to(self.device)

        # FIX: weights_only=False required for PyTorch >= 2.6
        # Safe to use because this checkpoint was saved by us.
    ...
    def predict(self, image):
        ...
        # Convert BGR → RGB before feeding to model
    ...
    def get_colored_segmentation(self, segmentation):
        """
        Convert segmentation mask to color-coded RGB image.

        Args:
            segmentation: 2D array with class IDs (0-3)

        Returns:
            colored: RGB image (safe for matplotlib.imshow)
        """
    ...
    def get_cost_map_visualization(self, cost_map):
        """
        Convert cost map to RGB colour visualization.
        ...
        """
        cost_viz = np.zeros((*cost_map.shape, 3), dtype=np.uint8)
        cost_viz[cost_map == 1.0]      = [0, 255, 0]    # Green  - floor
        cost_viz[cost_map == 0.8]      = [255, 255, 0]  # Yellow - door
        cost_viz[np.isinf(cost_map)]   = [255, 0, 0]    # Red    - blocked
    ...
    def process_image(self, image, save_prefix='output'):
        ...
        Args:
            image      : Input image (numpy array, BGR from cv2.imread)
        ...
        # Convert RGB → BGR before saving with cv2.imwrite
    ...
    def visualize(self, results, save_path=None):
        ...
        # Colored segmentation (RGB — safe for imshow)
        ...
        # Cost map visualization (RGB — safe for imshow)
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(
        description='Generate cost maps for indoor navigation')
