...
=====================================================
Generates segmentation masks and cost maps for A* path planning.

Classes:
    0: Floor (GREEN) - cost 1.0 (safe to traverse)
    1: Wall (RED) - cost inf (blocked)
    2: Door (YELLOW) - cost 0.8 (preferred path)
    3: No-go (MAGENTA) - cost inf (blocked)

Usage:
    from cost_map_generator import ColorCodedCostMapGenerator
    
    generator = ColorCodedCostMapGenerator('path/to/model.pth')
    results = generator.process_image('input_image.jpg')
    generator.visualize(results)
"""
...
class ColorCodedCostMapGenerator:
    ...
    # RGB Colors for visualization (BGR format for OpenCV)
    CLASS_COLORS = {
    ...
    def get_colored_segmentation(self, segmentation):
        ...
        Returns:
            colored: RGB image with class colors
        """
    ...
    def get_cost_map_visualization(self, cost_map):
        """
        Convert cost map to colored visualization.
        
        Args:
            cost_map: 2D array with cost values
        
        Returns:
            cost_viz: RGB visualization (green=good, yellow=door, red=blocked)
        """
        cost_viz = np.zeros((cost_map.shape[0], cost_map.shape[1], 3), dtype=np.uint8)
        cost_viz[cost_map == 1.0] = [0, 255, 0]      # Green - floor
        cost_viz[cost_map == 0.8] = [255, 255, 0]    # Yellow - door
        cost_viz[cost_map == float('inf')] = [255, 0, 0]  # Red - blocked
        return cost_viz
    ...
    def process_image(self, image, save_prefix='output'):
        ...
        cost_viz = self.get_cost_map_visualization(cost_map)
        
        # Save files
        np.save(f'{save_prefix}_cost_map.npy', cost_map)
        cv2.imwrite(f'{save_prefix}_colored_segmentation.png', colored_seg)
        cv2.imwrite(f'{save_prefix}_cost_viz.png', cost_viz)
    ...
    def print_statistics(self, results):
        ...
        blocked = np.sum(cost_map == float('inf'))
